...
download_date <- file.info(here('data', 'raw_data','GCEP','bold.xlsx'))$ctime %>% 
  ymd_hms() %>%
  format(., "%d %B %Y")

```

This is a basic summary of the BIN data that was available for download from BOLD for our projects GCEP and TMGHA, on `r download_date`. We have the following numbers of samples and BINs overall in our dataset:
...
As far as I understand it GCEP is our general dataset, TMGHA is our heath-traps. We have the following numbers of samples and BINs in each project:  
...
n_no_public_seqs <- sum(overall_bin_uniqueness$`Already sequenced, no public sequences`)
n_unique_to_us <- sum(overall_bin_uniqueness$`Unique to our project`)
```
  
And here is the data if we count BINs, rather than samples. `r n_no_public_seqs` BINs have been sequenced before but are not public, and `r n_unique_to_us` BINs are unique to our dataset. Opening our dataset to the public will thus represent `r n_no_public_seqs + n_unique_to_us` BINs entering the public domain for the first time
...
Ignoring data-availability as a variable, here's the taxonomic composition of our samples so far. Please note the log y-axis:
